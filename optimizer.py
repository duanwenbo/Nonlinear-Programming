...
class Optimizer:
    ...
    def conjugate_gradient(self):
        ...
        next_position_var = self._update_position( position_vector, current_position)
        obj_func = self.obj.subs(next_position_var)
        step_length = abs(self._golden_search(obj_func))
        next_position = self._update_position(position_vector, current_position, step_length)
        ...
        while not stop:
            ...
            next_position_var = self._update_position( new_position_vector, current_position)
            obj_func = self.obj.subs(next_position_var)
            step_length = abs(self._golden_search(obj_func))  # searching alpha
            next_position = self._update_position(new_position_vector, current_position, step_length)
            ...
            if self._modulus(position_vector) < self.EPSILON:
                stop = True
                current_position = [(i[0], "%.2f"%i[1]) for i in current_position]
    ...
    def _update_position(self, position_vector, current_position, step_length = 0):
        if step_length == 0:
            step_length = self.alpha
        assert len(self.start_position) == sp.shape(position_vector)[1], "check beta value!"
        ...
        result = [(self.start_position[i][0], (value[i])) for i in range(len(value))]
        return result

    def _grad(self,function, position):
        gradient = []
        for i in range(len(self.start_position)):
            gradient.append(function.diff(self.start_position[i][0]).subs(position))
...
if __name__=="__main__":
    ...
    opt = Optimizer(objective_function="(x_1 - 1)**2 + (x_2 -2)**2 + (x_3 - 3)**2",
